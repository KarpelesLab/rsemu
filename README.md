...
| Motorola 68010 / 68020 | the 68000 corpus, replayed on each | no public corpus exists for either, so every 68000 vector is run through the later model and **every disagreement must be a documented difference** — the 68010's format-`$8` frames, the 68020's misaligned operands and scaled index — or the test fails: **774,984** identical on the 68010 in state, cycles *and* trace, **731,457** on the 68EC020 in state, nothing unexplained. [`docs/cpu/m68k.md`](docs/cpu/m68k.md) has the classification |
