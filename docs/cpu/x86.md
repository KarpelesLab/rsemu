...
breakpoints (the registers round-trip and nothing fires), no 286-format task
state segment (`#TS` rather than a truncated switch), and no A20 gate — that
one is chipset, not CPU.
