...
only with `CR0.NE` set; no virtual-8086 mode, and with it no storage for
`EFLAGS.VIF` and `VIP` — the two bits that are storage only where `CR4.VME` or
`PVI` gives the processor virtual interrupts to keep, so a guest that found
them writable would be told a mechanism exists behind them (*Intel SDM* Vol 1
§3.4.3.3). `EFLAGS.ID` **is** storage, on every part whose `Features` include
