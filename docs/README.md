...
| [`mac-classic`](platforms/mac-classic.md) | 68000 | Apple's Macintosh Classic ROM | **as far as the insert-disk screen**, and it is one chip short of a booting Macintosh. The ROM sizes memory, initialises the SCC, sets its parameter RAM, resets the **Apple Desktop Bus** and walks all sixteen addresses asking each for register 3 - finding the keyboard at 2 and the mouse at 3 - probes the drive, sees that the mechanism is a **SuperDrive**, and asks the **SWIM** for ISM mode, then draws the grey desktop with the arrow cursor and the blinking floppy. A 1.44 MB image goes in, becomes IBM MFM cells and turns at 300 rpm under a head the ROM can step, every one of its 2,880 blocks making the round trip through the encoder. **ISM mode is not modelled** - no document available states its register file, and guessing one is the mistake `mac-plus` already paid for - so the chip cannot hand the ROM a sector. `docs/platforms/mac-classic.md` has the measured mode-register sequence to start from |
