...
| [`cpu/m68k.md`](cpu/m68k.md) | Motorola 68000, 68010, 68020/68EC020, 68030/68EC030 with its paged MMU, and 68040/68LC040/68EC040 with its on-chip FPU: the manuals, what each model implements and what it deliberately does not, the 68020's timing column, the **68881/68882 coprocessor** and why its transcendentals are computed better than the part's, and the corpus |
| [`cpu/other.md`](cpu/other.md) | PowerPC, SuperH, V850 |

### Machines
| | |
| --- | --- |
| [`platforms/amiga.md`](platforms/amiga.md) | Amiga 500, 500+, 600, **1200**, **CD32**, **3000**, **4000** and **4000T**: the memory map, the `OVL` overlay, the custom-chip register seam, the two CIAs, Paula's interrupts, disk, UART and audio, the floppy drive, the keyboard protocol and the mouse counters, ADF disks and their MFM tracks, Gayle and its IDE port, the Enhanced Chip Set, the **AA chip set** (Alice and Lisa), **Akiko** (the chunky-to-planar corner turn, the CD-ROM controller's registers and the EEPROM's two wires) with the CD-ROM drive and the joypad beside it, the A3000's **SCSI port** — a bus, a `scsi.disk` target, a **WD33C93A** and Commodore's **Super DMAC** — **Ramsey**, the A4000's **68040** with its motherboard fast RAM and its own **IDE port** at `$00DD2020` (found black-box, and how), the A4000T's **NCR 53C710** — a SCSI I/O Processor that fetches its own SCRIPTS out of guest memory and masters the bus for them, whose address, byte order and start byte were settled the same way — and what each document leaves open |
| [`platforms/mac-plus.md`](platforms/mac-plus.md) | Macintosh Plus: the memory map and the `ROMOVERLAY` overlay, why main memory **repeats** through its four-megabyte window and how that got settled twice, the 6522 on its A9-A12 decode, the video circuit that reads main memory, the Z8530, the clock chip's command encoding recovered off the wire, the keyboard's two shift-register modes, the IWM and its 800K drive with Apple's 6-and-2 GCR behind it, how far a real ROM gets — as far as the **insert-disk icon** — and what the screen shows, the ledger of what is missing *and what has been ruled out*, and why a Plus cannot read a 1.44 MB disk |
| [`platforms/mac-classic.md`](platforms/mac-classic.md) | Macintosh Classic: the same compact-Macintosh shape with a **512 KiB ROM** whose checksum only covers half of it, a **latching** overlay and why a board that let the ROM re-assert it died mid-instruction, why a word access to the VIA has to complete on a machine with no `/BERR`, the **Apple Desktop Bus** on the VIA's shift register - the state lines, the attention line and the whole byte handshake, established by measurement because no document here states it - the **SWIM** as the IWM's superset with a SuperDrive on its cable, IBM **MFM** and the 1.44 MB track with both address marks derived rather than quoted, the **ISM register set** out of Apple's *SWIM Chip User's Reference* rev. 1.5 quoted register by register, the `1, 0, 1, 1` mode switch and the `$57 $17 $57 $57` the ROM actually writes, the three things the document does not say and how each was measured - which status line says "SuperDrive", why the separator syncs on `$A1` and not `$C2`, and how the head gets chosen when the ROM never drives `HDSEL` - the four facts inferred rather than quoted, and the instruments that found all of it |
...
way; this table is the thing you cannot get by reading nine of them, which is
how they compare. **Every row is a measurement, not a gate**: no operating-system
boot here runs in CI, because each needs a kernel, a firmware image or a
machine's own ROM that rsemu does not ship (§1), and each is behind an
environment variable naming the directory it is read from — in place, never
copied in. The hermetic tests beside them — which do run on every `cargo test`
— drive the same hardware with a guest this repository builds.
...
| [`pc-at`](platforms/pc-at.md) | i386-class | FreeDOS 1.3, on firmware this repo assembles | **installed onto a hard disk and booted off it.** A person answers the Floppy Edition installer through the VNC input seam and swaps its five diskettes under the running guest: `FDISK` partitions a blank IDE drive, the machine reboots off the diskette, `FORMAT` writes FAT16, 114 archive volumes are unpacked, and the image the install wrote then boots on its own to `C:\>` with `VER` answering `FreeDOS 1.3`. It found three `INT 13h` defects on the way — the boot order, the EDD subset and the diskette change line — and, later, the two the memory manager in `FDCONFIG.SYS` needed: the privilege level in the window between `MOV CR0` and the far jump after it, and **virtual-8086 mode**, without which JemmEx could not run DOS at all. It does now, with five drivers loaded high and 173 KiB of upper memory free. The board has **real graphics** since: `pc.video`'s `vga` model is four planes, the four write modes and a scanout driven by the CRT controller rather than by a mode number, and the firmware programs modes 0Dh through 13h out of IBM's tables plus **VBE 2.0** over nine linear modes, whose framebuffer is read out of the adapter's BAR0. It also has a **CD-ROM** — an ATAPI packet device on the same cable as the disk — and **El Torito** in our own BIOS, with media type 4 declined rather than half-done and the disc last in `INT 19h`'s order so an installer cannot reboot into itself. `pc-at-smp` adds a second processor, but what runs on it is rsemu's own boot sector reading its APIC id back as `1` — **no operating system has run on two processors on this board**; the x86 board that now does is `q35-linux-smp`, interpreted as well as accelerated |
...
| [`q35-linux`](platforms/q35-linux.md) | x86-64 | the same, plus the chipset and a disk | a shell **on the board's own default command line**, reading bytes off an NVMe namespace through the kernel's own driver and a level-triggered interrupt. Its ledger is empty; **two of the four obstacles it named were refuted rather than fixed**, which is why the page is worth reading. `q35-linux-smp` is the same board with a second processor and one of the three here on which a real kernel does real SMP work — `smp: Brought up 1 node, 2 CPUs` at 1.7 s and `nproc` saying `2` at a shell at 2.8 s under KVM (see the caveat below), and the same line **interpreted** since `EFLAGS.ID` gained the storage that lets an application processor's real-mode trampoline find its own `CPUID` at all. Also the only board measured **under KVM**: the same boot in **2.4 seconds** of wall clock against **978** interpreted, 282 of 346 console lines byte-identical — and on the same command line either way, because the `no_timer_check` this row used to name is gone along with the defect it hid |
...
| [`amiga-a500`](platforms/amiga.md), `-a500plus`, `-a600`, `-a1200` | 68000, 68EC020 | Kickstart 1.3 / 2.04 / 3.1, Workbench, AROS | **a desktop you can use.** Workbench 1.3 and 2.04 boot to their desktops off a real ADF on the A500, and a test then double-clicks the disk icon, opens a Shell and types `echo hello` into it through the same input seam a VNC client drives; Workbench 2.04 boots on the A500+'s Enhanced Chip Set; the A600 boots off an IDE hard disk on Gayle; the A1200 boots Workbench 3.1 off both the disk and a floppy, with `graphics.library` reading the AA bits back in `GfxBase->ChipRevBits0` where the A600 beside it reads only the ECS pair. AROS boots its own disk to the Workbook desktop, given the extended-ROM socket and more memory than Commodore sold. The four audio channels reach a host `.wav` in stereo |
| [`amiga-a3000`](platforms/amiga.md), `-a4000`, `-a4000t` | 68030 + 68882, 68040 | Kickstart 2.04 / 3.1 / 3.X, Workbench 2.1 and 3.1 | **Workbench off a hard disk on three cables.** The A3000 boots Workbench 3.1 under Kickstart 3.1 and Workbench 2.1 under 2.04, off a **SCSI** disk behind a WD33C93A and Commodore's Super DMAC; the A4000 boots 3.1 off its own **IDE** port, at an address Commodore never published and found by recording what the ROM touches; the A4000T boots 3.1 off **either**, its SCSI side an **NCR 53C710** running SCRIPTS out of guest memory. In each case the *guest* is what reports the chip set, the processor, an `ExecBase` relocated into motherboard fast RAM and a `DH0` task for the partition it found in the Rigid Disk Block. Where it stops: the A3000's fast RAM must be **top-aligned** at `$08000000` or the ROM's downward sizing routine finds none of it; the 53C710's target role, reselection from the cable, synchronous transfer and parity are not modelled, and the ledger names each |
| [`amiga-cd32`](platforms/amiga.md) | 68EC020 | a two-part Kickstart 3.1 (40.60) | **the animated boot screen**, which is what a real CD32 with an empty tray does — a starfield, a turning disc and the "AMIGA CD32" logo under a sky that sweeps behind it, with the processor stopped from 6 s on and the copper and blitter doing the work. `cd.device` finds **Akiko**, proves its chunky-to-planar corner turn and talks to the EEPROM over two wires. **A disc in the tray changes nothing, hash for hash**, and that is the gap rather than the result: Akiko carries no disc-change message, so the guest is never told the tray moved. Amiga Forever ships no CD32-bootable disc and nothing here went looking for a game |
| [`mac-plus`](platforms/mac-plus.md) | 68000 | Apple's Macintosh Plus ROM | **as far as the blinking insert-disk icon**, and not past it. The ROM chimes, sizes its megabyte, runs its memory test, finds the clock chip's battery flat and writes parameter RAM back, finishes the keyboard's Model Number handshake, draws the grey desktop with the arrow cursor, and then polls the drive six to eight times a second. Put an 800K image in and it starts the motor, takes two runs at reading it and puts it back out with the unreadable-disk cross. **It does not get a track off a disk with the ROM driving** — after the motor starts the ROM reads the tachometer in a tight loop for six virtual seconds and gives up — and that is ledger item 1. The drive itself reads: an image becomes 6-and-2 GCR bit cells and the IWM shifts them past its head, checked through the chip |
| [`mac-classic`](platforms/mac-classic.md) | 68000 | Apple's Macintosh Classic ROM | **boots Mac OS 6.0.8 off a 1.44 MB disk to the Finder desktop**: the happy Mac at about ten virtual seconds, "Welcome to Macintosh" at fifteen, and by seventy a desktop with the menu bar, the startup volume's icon in the top right corner and the Trash in the bottom right. The ROM sizes memory, initialises the SCC, sets its parameter RAM, resets the **Apple Desktop Bus** and walks all sixteen addresses asking each for register 3 - finding the keyboard at 2 and the mouse at 3 - probes the drive, sees that the mechanism is a **SuperDrive**, and switches the **SWIM** into **ISM mode**, whose register set is Apple's *SWIM Chip User's Reference* rev. 1.5 quoted register by register. With an empty drive it draws the blinking insert-disk icon instead. `docs/platforms/mac-classic.md` has every quotation with its page, the four things inferred rather than quoted, and the measurements the document does not cover - which drive status line says "SuperDrive", why the separator syncs on `$A1` and not `$C2`, and why it must take the first of a field's three sync bytes |
...
| [`devices/pc-video.md`](devices/pc-video.md) | `pc.video`: the VGA's planar memory and its scanout, and the part that is **ours** — the extension registers a linear framebuffer is set up through, and the VBE services the in-house BIOS builds on them. Deliberately not Bochs' DISPI interface, whose only specification is a GPL program's source |
...
| [`system/monitor.md`](system/monitor.md) | The monitor console: an interactive prompt on a stopped machine — the device tree and one device's whole current state, the clock forest, the wire graph, the scheduler's queue, memory read through `MemAttrs::debug`, snapshots, rewind and the counters live. Why slicing an advance cannot move the state hash, why eight of its commands are answered by the gdbstub's code, and — since `media`, `insert` and `eject` reach every removable bay on the board — how a swap stays deterministic when the thing being swapped is a host action crossing into the machine |
...
| [`techniques/a64-code-generation.md`](techniques/a64-code-generation.md) | Emitting AArch64 specifically: why fixed-width encoding is the easy half and a non-coherent instruction cache is the hard one. Written while `src/jit/arm64` was built, and the record of which decisions came from the manual and which from measurement |
| [`techniques/wasm-jit.md`](techniques/wasm-jit.md) | The wasm backend: lowering an IR block to a `WebAssembly.Module`, the browser embedder's three imports and four exports, and the measurement — 1.50× the IR interpreter in V8, against a `ROADMAP.md` §11.4 that doubted it would win at all. The browser column moves run to run (1.48× to 1.60× over five), which is why `web/check.mjs` prints the ratio instead of gating on it |
| [`techniques/clock-control.md`](techniques/clock-control.md) | How a device re-rates a clock domain at runtime, why the change lands on a scheduling boundary rather than where the guest wrote the register, and what becomes of the ticks already counted |
...
| [`../fuzz/README.md`](../fuzz/README.md) | The **twenty-nine** `cargo-fuzz` targets and the property each asserts — a separate crate, detached from the workspace, so `libfuzzer-sys` never reaches `rsemu`'s dependency tree. Also the phase-2 gate's exact command: a CPU-hour on the `.machine` parser from a seeded corpus, because unbounded fuzzing is never "clean" and a stated budget is |
...
`docs/upstream/` is not a source register at all — it is where a defect in a
*sibling* repository is written down. rsemu's working rule is that we do not
push to sibling repos, so an upstream fix becomes a **specification** somebody
carries across deliberately, rather than a commit made in passing to a tree
nobody here owns. Each page says what happens, what the fix is, and what rsemu
