...
| [`platforms/mac-plus.md`](platforms/mac-plus.md) | Macintosh Plus: the memory map and the `ROMOVERLAY` overlay, why main memory must **not** repeat, the 6522 on its A9-A12 decode, the video circuit that reads main memory, the Z8530, the clock chip's command encoding recovered off the wire, the keyboard's two shift-register modes, the IWM and its 800K drive with Apple's 6-and-2 GCR behind it, how far a real ROM gets and what the screen shows, the ledger of what is missing *and what has been ruled out*, and why a Plus cannot read a 1.44 MB disk |
