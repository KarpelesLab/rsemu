...
**A read of a free-running counter does not pay that price**, and it is worth
being exact about why not. Catch-up is what the rule above restricts, because
...
`LazyHandle::reader_tick`): the exact tick count its own position corresponds
to in each such device's domain, built before anything runs and from nobody
