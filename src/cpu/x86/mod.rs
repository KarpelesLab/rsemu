...
impl X86 {
    ...
    fn advance(&self, remaining: u64) -> u64 {
        ...
        if self.engine.translates() {
            ...
            // translation buffer so the two evict together
            // (`paging::Tlb::attach_shadow`). It is asked for here rather than
            // at construction because it needs the address space, which
            // arrives with a bind, and only the engine that reads it asks for
            // it at all.
            if !session.state.tlb.has_shadow()
                && session.jit.as_ref().is_some_and(|jit| jit.wants_shadow())
                && let Some(space) = session.memory.clone()
            {
                session.state.tlb.attach_shadow(space);
            }
            let Session {
                state,
                memory,
