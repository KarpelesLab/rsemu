...
struct Contents {
    ...
    /// stay. `machines/riscv-virt.machine` maps `boot` into `mem` at
    /// `0x1000`, which closes it.
    ///
...
mod tests {
    ...
    /// space's map, as `machines/riscv-virt.machine` maps `boot` at `0x1000`,
    /// and the space in the ROM's contents, as `bind` puts it there.
    #[test]
    fn the_rom_does_not_keep_the_space_it_describes_alive() {
        let rom = BootRom::new(&Props::new()).expect("a boot ROM with every default");
        let space = Arc::new(AddressSpace::new("mem", 64));
        space
            .topology()
            .map(rom.region("").expect("the ROM window"), 0x1000)
