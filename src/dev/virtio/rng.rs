...
//! # The generator
//!
//! [`core::rand::Stream`](crate::core::rand::Stream), which is SplitMix64 —
//! Steele, Lea and Flood, *Fast Splittable Pseudorandom Number Generators*
//! (OOPSLA 2014). It used to be written out here, and moved when `st.rng`
//! needed the same four lines and the same determinism argument; the byte
//! stream and the snapshot encoding are unchanged.
//!
//! What this device does **not** do is
//! [`derive_seed`](crate::core::rand::derive_seed) its stream from its instance
//! path the way `st.rng` does. It predates that convention and its seed is
//! written into four shipped machine files, so deriving here would silently
//! change what every one of those boards hands its guest.

use alloc::vec::Vec;

use crate::core::error::Result;
use crate::core::rand::Stream;
...
impl VirtioRng {
    ...
    pub fn new(seed: u64) -> VirtioRng {
        VirtioRng {
            stream: Mutex::with_rank(LockRank::DEVICE, Stream::new(seed)),
        }
    }
...
impl Backend for VirtioRng {
    ...
    fn save(&self, w: &mut ChunkWriter<'_>) -> Result<()> {
        ...
        self.stream.lock().save(w)
    }

    fn load(&self, r: &mut ChunkReader<'_>) -> Result<()> {
        self.stream.lock().load(r)
    }
}
